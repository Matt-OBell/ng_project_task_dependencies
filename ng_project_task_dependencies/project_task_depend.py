...
class project_task_prev(osv.Model):
    _name = "project.task.previous"

    _columns = {
        'depend_type': fields.selection([('none', 'None (Unused)'), 
                                         ('fts','Finish To Start'),#
                                          ('stf','Start To Finish (Unused)'), 
                                          ('sts','Start To Start'), #
                                          ('ftf','Finish To Finish (Unused)')
                                          ], 'Type of Dependency', required=True ,
        ...
        'prev_task_id': fields.many2one('project.task', 'Previous Task', required=True),
        
        'rel_task_id': fields.many2one('project.task', 'Next Task', required=False, readonly=0),
    }
...
#        
        'previous_task_ids_new': fields.one2many('project.task.previous', 'rel_task_id', 'Previous Tasks Dependencies'),
        'next_task_ids_new': fields.one2many('project.task.previous', 'prev_task_id', 'Next Tasks Dependencies'),
...
#        Basically Lead is overlap and Lag is delay.
        ...
        if vals.get('stage_id',False):
            ...
            if task.previous_task_ids_new:
                for prev in task.previous_task_ids_new:
                    #if Finish to Start doesn't have lead or lag time it check only state
                    if prev.depend_type == 'fts' and prev.lead_time == 0.0 and prev.lag_time == 0.0: #Finish to Start
                        if prev.prev_task_id.stage_id.state != 'done' and stage_my.state in ['open','done']:
                    ...
                    elif prev.depend_type == 'fts' and prev.lead_time == 0.0 and prev.lag_time > 0.0:
                        end_date = datetime.datetime.strptime(prev.prev_task_id.date_end, "%Y-%m-%d %H:%M:%S")
                        end_date_timedelta = end_date + datetime.timedelta(hours = prev.lag_time)
                        if end_date_timedelta > datetime.datetime.now():
                            if prev.prev_task_id.stage_id.state != 'done' and stage_my.state == 'open':
                ...
                for prev in task.previous_task_ids_new:
                    if prev.depend_type == 'sts': #Start to Start
                        if prev.prev_task_id.stage_id.state == 'draft' and stage_my.state == 'open':
